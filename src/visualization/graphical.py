...
class Visualizer:
    def __init__(self, graph: Graph, turns: List[str]):
        ...
        # Sidebar UI
        tk.Label(self.sidebar, text="NEON ROUTING LOGIC", font=self.font_title, bg=self.sidebar_bg, fg=self.text_color).pack(pady=(40, 20))
        
        self.lbl_turn = tk.Label(self.sidebar, text="TURN 00", font=self.font_turn, bg=self.sidebar_bg, fg=self.text_color)
        self.lbl_turn.pack(pady=20)
        
        def create_button(text, command, primary=False):
            btn = tk.Button(self.sidebar, text=text, font=("Helvetica", 12, "bold"), 
        ...
        self.btn_auto = create_button("▶ INITIATE SEQUENCE", self.toggle_auto_play, primary=True)
    ...
    def get_coords(self, x: float, y: float):
        ...
        scale = min(canvas_w / (self.graph_w + 2), canvas_h / (self.graph_h + 2)) if (self.graph_w > 0 or self.graph_h > 0) else 50
        scale *= 0.85
        offset_x = (canvas_w - (self.graph_w * scale)) / 2
        offset_y = (canvas_h - (self.graph_h * scale)) / 2
        
        return offset_x + (x - self.min_x) * scale, offset_y + (y - self.min_y) * scale
    ...
    def draw_graph(self):
        ...
        for x in range(0, w, 60):
            for y in range(0, h, 60):
                self.canvas.create_oval(x, y, x+1, y+1, fill="#1F2833", outline="", tags="graph")
        
        # Connections
        for conn in self.graph.connections:
            x1, y1 = self.get_coords(conn.zone_a.x, conn.zone_a.y)
            x2, y2 = self.get_coords(conn.zone_b.x, conn.zone_b.y)
            self.canvas.create_line(x1, y1, x2, y2, fill=self.conn_color, width=6, tags="graph", capstyle=tk.ROUND, smooth=True)
            self.canvas.create_line(x1, y1, x2, y2, fill="#303C4B", width=2, tags="graph", capstyle=tk.ROUND, smooth=True)
            
        # Zones
        for name, zone in self.graph.zones.items():
            x, y = self.get_coords(zone.x, zone.y)
            
            if zone.zone_type == "blocked": color = self.zone_blocked
            elif zone.zone_type == "restricted": color = self.zone_restricted
            else: color = self.zone_normal
            
            try:
                # Glowing outer ring
                self.canvas.create_oval(x-24, y-24, x+24, y+24, outline=color, width=2, tags="graph")
                # Solid inner core
                self.canvas.create_oval(x-10, y-10, x+10, y+10, fill=color, outline="", tags="graph")
            except tk.TclError:
                self.canvas.create_oval(x-24, y-24, x+24, y+24, outline=self.text_color, width=2, tags="graph")
                
            self.canvas.create_text(x, y-35, text=name.upper(), fill=self.text_muted, font=self.font_small, tags="graph")
    ...
    def draw_drone(self, d_id, cx, cy, angle):
        ...
        # Draw dart body
        shapes.append(self.canvas.create_polygon(px1, py1, px2, py2, px3, py3, px4, py4, fill=self.drone_color, outline=self.bg_color, width=1))
        # Draw ID text
        shapes.append(self.canvas.create_text(cx, cy-20, text=d_id, fill=self.text_color, font=("Helvetica", 8, "bold")))
    ...
    def draw_drones(self):
        ...
        for (x, y), drones in pos_counts.items():
            ...
            for i, d_id in enumerate(drones):
                ...
                angle = self.drone_angles.get(d_id, -math.pi / 2) # Default pointing up
                self.draw_drone(d_id, base_cx + offset_x, base_cy + offset_y, angle)

    def retry(self):
        self.is_auto_playing = False
        self.current_turn = -1
        self.lbl_turn.config(text="TURN 00")
        self.btn_auto.config(text="▶ INITIATE SEQUENCE", bg=self.text_color, fg=self.bg_color)
    ...
    def animate_step(self, frame, total_frames, start_positions):
        ...
        if frame < total_frames:
            self.root.after(30, lambda: self.animate_step(frame + 1, total_frames, start_positions))
        else:
            self.btn_next.config(state=tk.NORMAL)
    ...
    def next_turn(self):
        ...
        for move in movements:
            ...
            if len(parts) == 2:
                ...
                # This bypasses the midpoint entirely so they never look stuck in the connection!
    ...
    def toggle_auto_play(self):
        ...
        if self.is_auto_playing:
            self.btn_auto.config(text="⏸ HALT SEQUENCE", bg="#FF007A", fg="#FFFFFF")
            self.next_turn()
        else:
            self.btn_auto.config(text="▶ INITIATE SEQUENCE", bg=self.text_color, fg=self.bg_color)
